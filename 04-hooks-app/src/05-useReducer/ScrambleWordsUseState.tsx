...
import confetti from 'canvas-confetti';   // ctrl + . y luego instalar el tipado 
...
export const ScrambleWords = () => {
  const [words, setWords] = useState(shuffleArray(GAME_WORDS));     // shuffleArray mezcla el arreglo.

  const [currentWord, setCurrentWord] = useState(words[0]);         // Palabra a adivinar
  ...
  const handleGuessSubmit = (e: React.FormEvent) => {
    ...
    // console.log('Intento de adivinanza:', guess, currentWord);   // guess = Lo que la persona escribió // currentWord = Palabra a adivinar

    if (guess === currentWord) {    // Lo que la persona escribe es igual a la palabra a adivinar
        ...
        setScrambledWord(scrambleWord(newWords[0]));    // scrambleWord - Mezcla las letras
        return;
    }
  ...
  const handleSkip = () => {
    ...
    const updateWords = words.splice(1);    // Recorto la palabra para pasar a la siguiente

    setSkipCounter( skipCounter + 1);       // Seteo el contador de veces que se pueden Saltar
    setWords( updateWords );                // Seteo el arreglo nuevo de la palabras que quedan
    setCurrentWord( updateWords[0] );       // Seteo la palabra a adivinar con la 1° posición del arreglo nuevo
    setScrambledWord( scrambleWord(updateWords[0]) );    // Reordeno las letras de la palabra para adivinar
    setGuess('');
  ...
  const handlePlayAgain = () => {
    ...
    setPoints(0);       // Cantidad de palabras adivinadas.
    setErrorCounter(0);
    setGuess('');
    setWords( newArray );
